...
data = pd.read_csv('large_tiles_f5.csv', header=None)
task = []
...
src='/its/home/mc741/git_hub/dmu_products/dmu26/dmu26_XID+SPIRE_Herschel-Stripe-82/XID_plus_hier.sh'
for index, name in enumerate(folders):
        ...
        f.write(s)
        f.close()
        print('qsub -t 1-%s-l m_mem_free=20G -q seb_node.q XID_plus_hier.sh' % task[0][index])
        os.system('qsub -t 1-%s -l m_mem_free=20G -q seb_node.q XID_plus_hier.sh' % task[0][index])
