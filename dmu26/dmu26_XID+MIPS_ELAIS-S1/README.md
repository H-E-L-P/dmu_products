...
```
Then fit all the main tiles, where $n_tiles is the number of main tiles. Each tile requires 4 cores, 10GB memory and estimated to run for 4 hours:
...
stilts tcat ifmt=fits in=@cat_files out=dmu26_XID+MIPS_ELAIS-S1_cat.fits
```
...
### Final data products
  ...
  Final stage requires examination and validation of catalogues using [XID+MIPS_ELAIS-S1_final_processing.ipynb](XID+MIPS_ELAIS-S1_final_processing.ipynb).
