...
im100=im100*2.35045e-2*(np.abs(hdulist[1].header['CDELT1'])*np.abs(hdulist[1].header['CDELT2']))*3600**2
nim100=nim100*2.35045e-2*(np.abs(hdulist[1].header['CDELT1'])*np.abs(hdulist[1].header['CDELT2']))*3600**2
...
# Convert units MJy/sr to mJy/pixel: http://coolwiki.ipac.caltech.edu/index.php/Units
im160=im160*2.35045e-2*(np.abs(hdulist[1].header['CDELT1'])*np.abs(hdulist[1].header['CDELT2']))*3600**2
nim160=nim160*2.35045e-2*(np.abs(hdulist[1].header['CDELT1'])*np.abs(hdulist[1].header['CDELT2']))*3600**2
...
import pylab as plt
plt.hist(im100.flatten(),bins=np.arange(-50.0,100.0,0.5));
plt.yscale('log')
...
pacs100_psf=fits.open('../../dmu18/dmu18_XMM-LSS/dmu18_PACS_100_PSF_XMM-LSS_20190122.fits')
pacs160_psf=fits.open('../../dmu18/dmu18_XMM-LSS/dmu18_PACS_160_PSF_XMM-LSS_20190122.fits')
