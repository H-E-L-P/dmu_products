...
;---------------------------------
   ...
   fwhm = 6. ; estimate FWHM of beam only used to decide approx size of stack (4XFWHM)^2
...
CASE format OF  
   ...
   'pacs':  BEGIN 
      ...
      IF keyword_set(cov_file) THEN cov = readfits(cov_file,cov_hd)
       ...
       im=mrdfits(im_file,0,hd, /silent)
       cov=mrdfits(im_file,0, error_action=1, cov_hd, /silent)
       IF sxpar(cov_hd, 'naxis') EQ 0 THEN BEGIN
          cov = im*0.+1.
          cov_hd=hd
       ENDIF
       err=mrdfits(im_file,0, error_action=1, err_hd, /silent)
